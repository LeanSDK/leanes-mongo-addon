...
export default (Module) => {
  ...
  Module.defineMixin(__filename, (BaseClass) => {
    ...
    > extends BaseClass {
      ...
      @property _cursorFactory: (?string, A, ?string) => CursorInterface<C, D, A>;
      ...
      @method async takeMany(ids: Array<string | number>): Promise<CursorInterface<C, D, A>> {
        const result = await this.adapter.takeMany(this.delegate, ids);
        return this._cursorFactory(this.getName(), result, 'MongoCursor');
      }

      @method async takeAll(): Promise<CursorInterface<C, D, A>> {
